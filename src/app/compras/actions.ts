...
export async function createPurchaseRequest(formData: FormData) {
  ...
  const request = await prisma.purchaseRequest.create({
    data: {
      folio: createFolio("REQ"), requester: data.requester, status: "REQUESTED",
      neededBy: data.neededBy ? new Date(`${data.neededBy}T12:00:00`) : null, notes: data.notes,
...
export async function createPurchaseOrder(formData: FormData) {
  ...
  const order = await prisma.purchaseOrder.create({
    data: { folio: createFolio("OC"), supplierId, status: "ISSUED", expectedAt: expectedAtRaw ? new Date(`${String(expectedAtRaw)}T12:00:00`) : null, subtotal, tax, total, items: { create: { description, chemicalId, quantity, unit, unitCost, taxRate, subtotal, tax, total } } }
  });
...
export async function receivePurchaseOrder(formData: FormData) {
  ...
  const result = await prisma.$transaction(async tx => {
    await tx.$queryRaw`SELECT id FROM "PurchaseOrderItem" WHERE id = ${itemId} FOR UPDATE`;
    ...
    if (quantity > pending) throw new Error("La recepción excede la cantidad pendiente. Otro usuario pudo haber recibido parte de la orden; actualiza la pantalla.");
    if (item.chemicalId && !warehouseId) throw new Error("Selecciona almacén para recibir el químico.");

    const receipt = await tx.purchaseReceipt.create({
      data: { folio: createFolio("REC"), purchaseOrderId: order.id, supplierId: order.supplierId, warehouseId, items: { create: { purchaseOrderItemId: item.id, quantity, lotNumber, expiresAt: expiresAtRaw ? new Date(`${String(expiresAtRaw)}T12:00:00`) : null } } }
    });

    const updatedItem = await tx.purchaseOrderItem.update({ where: { id: item.id }, data: { receivedQuantity: { increment: quantity } } });

    let chemicalLotId: string | null = null;
    if (item.chemicalId && warehouseId) {
      const chemLotNumber = lotNumber || createFolio("QLT");
      const chemicalLot = await tx.chemicalLot.upsert({
        where: { chemicalId_warehouseId_lotNumber: { chemicalId: item.chemicalId, warehouseId, lotNumber: chemLotNumber } },
        update: { currentQuantity: { increment: quantity }, initialQuantity: { increment: quantity } },
        create: { chemicalId: item.chemicalId, warehouseId, supplierId: order.supplierId, lotNumber: chemLotNumber, initialQuantity: quantity, currentQuantity: quantity, unitCost: item.unitCost, expiresAt: expiresAtRaw ? new Date(`${String(expiresAtRaw)}T12:00:00`) : null, notes: `Recepción ${receipt.folio} de ${order.folio}` }
      });
    ...
    return { receipt, orderStatusBefore: order.status, orderStatusAfter: updatedOrder.status, chemicalLotId, description: item.description, newReceived: Number(updatedItem.receivedQuantity) };
  });
