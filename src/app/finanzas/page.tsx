...
export default async function FinancePage() {
  ...
  return <>
    ...
    <h2>Cuentas por cobrar</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Cliente</th><th>Total</th><th>Saldo</th><th>Vence</th><th>Cobrar</th></tr></thead><tbody>{receivables.length===0?<tr><td colSpan={6} className="muted">Sin cuentas por cobrar.</td></tr>:receivables.map(r=><tr key={r.id}><td>{r.folio}</td><td>{customerMap.get(r.customerId)??r.customerId}</td><td>${Number(r.total).toFixed(2)}</td><td>${Number(r.balance).toFixed(2)}</td><td>{r.dueDate?.toLocaleDateString("es-MX")??"—"}</td><td>{Number(r.balance)>0?<form action={registerReceivablePayment} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="receivableId" value={r.id}/><input name="amount" type="number" step="0.01" max={Number(r.balance)} defaultValue={Number(r.balance)} required/><select name="method">{methods.map(m=><option key={m} value={m}>{m}</option>)}</select><input name="reference" placeholder="Referencia"/><button className="button">Registrar cobro</button></form>:<span className="badge">PAGADA</span>}</td></tr>)}</tbody></table></div>

    <h2>Cuentas por pagar</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Proveedor</th><th>Total</th><th>Saldo</th><th>Vence</th><th>Pagar</th></tr></thead><tbody>{payables.length===0?<tr><td colSpan={6} className="muted">Sin cuentas por pagar.</td></tr>:payables.map(r=><tr key={r.id}><td>{r.folio}</td><td>{supplierMap.get(r.supplierId)??r.supplierId}</td><td>${Number(r.total).toFixed(2)}</td><td>${Number(r.balance).toFixed(2)}</td><td>{r.dueDate?.toLocaleDateString("es-MX")??"—"}</td><td>{Number(r.balance)>0?<form action={registerPayablePayment} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="supplierInvoiceId" value={r.id}/><input name="amount" type="number" step="0.01" max={Number(r.balance)} defaultValue={Number(r.balance)} required/><select name="method">{methods.map(m=><option key={m} value={m}>{m}</option>)}</select><input name="reference" placeholder="Referencia"/><button className="button">Registrar pago</button></form>:<span className="badge">PAGADA</span>}</td></tr>)}</tbody></table></div>
