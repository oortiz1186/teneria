...
import { cancelReceivable, cancelSupplierInvoice, createExpense, createReceivable, registerPayablePayment, registerReceivablePayment, reversePayment } from "./actions";
...
export default async function FinancePage() {
  const [customers, suppliers, orders, shipments, receivables, payables, payments, expenses] = await Promise.all([
    ...
    prisma.shipment.findMany({ where: { status: { not: "CANCELLED" } }, orderBy: { createdAt: "desc" }, take: 30 }),
    prisma.accountReceivable.findMany({ include: { payments: true }, orderBy: { issuedAt: "desc" }, take: 30 }),
    prisma.supplierInvoice.findMany({ include: { payments: true }, orderBy: { issuedAt: "desc" }, take: 30 }),
    prisma.payment.findMany({ include: { applications: true }, orderBy: { paidAt: "desc" }, take: 30 }),
    prisma.expense.findMany({ orderBy: { occurredAt: "desc" }, take: 20 })
  ]);

  const customerMap = new Map(customers.map(c => [c.id, c.name]));
  const supplierMap = new Map(suppliers.map(s => [s.id, s.name]));
  const arTotal = receivables.filter(r=>r.status!=="CANCELLED").reduce((a,r)=>a+Number(r.balance),0);
  const apTotal = payables.filter(r=>r.status!=="CANCELLED").reduce((a,r)=>a+Number(r.balance),0);
  const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0,0,0,0);
  const monthExpenses = expenses.filter(e=>e.occurredAt>=monthStart && e.status!=="CANCELLED").reduce((a,e)=>a+Number(e.amount)+Number(e.tax),0);
  ...
  return <>
    ...
    <h2>Cuentas por cobrar</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Cliente</th><th>Total</th><th>Saldo</th><th>Estado</th><th>Documentos</th><th>Cobrar</th><th>Cancelar</th></tr></thead><tbody>{receivables.length===0?<tr><td colSpan={8} className="muted">Sin cuentas por cobrar.</td></tr>:receivables.map(r=><tr key={r.id}><td><Link href={`/finanzas/cxc/${r.id}`}>{r.folio}</Link></td><td>{customerMap.get(r.customerId)??r.customerId}</td><td>${Number(r.total).toFixed(2)}</td><td>${Number(r.balance).toFixed(2)}</td><td><span className="badge">{r.status}</span></td><td><Link className="button button-secondary" href={`/finanzas/cxc/${r.id}`}>CFDI / PDF</Link></td><td>{r.status!=="CANCELLED"&&Number(r.balance)>0?<form action={registerReceivablePayment} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="receivableId" value={r.id}/><input name="amount" type="number" step="0.01" max={Number(r.balance)} defaultValue={Number(r.balance)} required/><select name="method">{methods.map(m=><option key={m} value={m}>{m}</option>)}</select><input name="reference" placeholder="Referencia"/><button className="button">Registrar cobro</button></form>:<span className="muted">—</span>}</td><td>{r.status!=="CANCELLED"?<form action={cancelReceivable} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="receivableId" value={r.id}/><input name="reason" minLength={10} maxLength={500} placeholder={r.payments.length?"Reversa cobros primero":"Motivo de cancelación"} disabled={r.payments.length>0} required/><button className="button button-secondary" disabled={r.payments.length>0}>Cancelar CxC</button>{r.payments.length>0&&<span className="muted">Tiene cobros aplicados.</span>}</form>:<span className="muted">Cancelada</span>}</td></tr>)}</tbody></table></div>

    <h2>Cuentas por pagar</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Proveedor</th><th>Total</th><th>Saldo</th><th>Estado</th><th>Documentos</th><th>Pagar</th><th>Cancelar</th></tr></thead><tbody>{payables.length===0?<tr><td colSpan={8} className="muted">Sin cuentas por pagar.</td></tr>:payables.map(r=><tr key={r.id}><td><Link href={`/compras/facturas/${r.id}`}>{r.folio}</Link></td><td>{supplierMap.get(r.supplierId)??r.supplierId}</td><td>${Number(r.total).toFixed(2)}</td><td>${Number(r.balance).toFixed(2)}</td><td><span className="badge">{r.status}</span></td><td><Link className="button button-secondary" href={`/compras/facturas/${r.id}`}>XML / PDF</Link></td><td>{r.status!=="CANCELLED"&&Number(r.balance)>0?<form action={registerPayablePayment} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="supplierInvoiceId" value={r.id}/><input name="amount" type="number" step="0.01" max={Number(r.balance)} defaultValue={Number(r.balance)} required/><select name="method">{methods.map(m=><option key={m} value={m}>{m}</option>)}</select><input name="reference" placeholder="Referencia"/><button className="button">Registrar pago</button></form>:<span className="muted">—</span>}</td><td>{r.status!=="CANCELLED"?<form action={cancelSupplierInvoice} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="supplierInvoiceId" value={r.id}/><input name="reason" minLength={10} maxLength={500} placeholder={r.payments.length?"Reversa pagos primero":"Motivo de cancelación"} disabled={r.payments.length>0} required/><button className="button button-secondary" disabled={r.payments.length>0}>Cancelar factura</button>{r.payments.length>0&&<span className="muted">Tiene pagos aplicados.</span>}</form>:<span className="muted">Cancelada</span>}</td></tr>)}</tbody></table></div>
    ...
    <h2>Pagos y cobros recientes</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Tipo</th><th>Método</th><th>Importe</th><th>Fecha</th><th>Referencia</th><th>Reversa</th></tr></thead><tbody>{payments.length===0?<tr><td colSpan={7} className="muted">Sin movimientos.</td></tr>:payments.map(p=><tr key={p.id}><td>{p.folio}</td><td>{p.direction}</td><td>{p.method}</td><td>${Number(p.amount).toFixed(2)}</td><td>{p.paidAt.toLocaleString("es-MX")}</td><td>{p.reference??"—"}</td><td>{p.applications.length===1?<form action={reversePayment} style={{display:"grid",gap:6,minWidth:220}}><input type="hidden" name="paymentId" value={p.id}/><input name="reason" minLength={10} maxLength={500} placeholder="Motivo de reversa" required/><button className="button button-secondary">Reversar</button></form>:<span className="muted">No reversible desde aquí</span>}</td></tr>)}</tbody></table></div>
