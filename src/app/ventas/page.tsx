...
export default async function SalesPage() {
  const [customers, products, routes, quotes, orders, completedLots, shipments] = await Promise.all([
    ...
    prisma.salesOrder.findMany({ include: { customer: true, items: { include: { product: true } }, productionOrders: true, shipments: true }, orderBy: { createdAt: "desc" }, take: 20 }),
  ...
  return <>
    ...
    <h2>Pedidos</h2><div className="table-wrap" style={{marginBottom:22}}><table><thead><tr><th>Folio</th><th>Cliente</th><th>Total</th><th>Estado</th><th>OP</th><th>Acción</th></tr></thead><tbody>{orders.length===0?<tr><td colSpan={6} className="muted">Sin pedidos.</td></tr>:orders.map(o=><tr key={o.id}><td>{o.folio}</td><td>{o.customer.name}</td><td>${Number(o.total).toFixed(2)}</td><td><span className="badge">{o.status}</span></td><td>{o.productionOrders.map(p=>p.folio).join(", ")||"—"}</td><td>{o.status==="DRAFT"&&<form action={confirmSalesOrder}><input type="hidden" name="salesOrderId" value={o.id}/><button className="button">Confirmar y enviar a producción</button></form>}</td></tr>)}</tbody></table></div>

    <div className="card" style={{marginBottom:22}}><h2>Generar remisión</h2><form action={createShipment} className="form">
