...
export default async function QualityPage() {
  const [lots, inspections, defects] = await Promise.all([
    prisma.tanneryLot.findMany({ where: { status: { in: ["IN_PROCESS", "ON_HOLD", "COMPLETED"] } }, orderBy: { updatedAt: "desc" } }),
  ...
  return <>
    <div className="header"><div><h1 className="title">Calidad</h1><div className="muted">Inspecciones, defectos, clasificación, pruebas, evidencia, liberación y reproceso trazable.</div></div></div>
    ...
    <div className="card" style={{ marginBottom: 22 }}><h2>Nueva inspección</h2><form action={createInspection} className="form">
    ...
    {inspections.length === 0 ? <div className="card muted">Aún no hay inspecciones.</div> : inspections.map(i => {
      ...
      const routeSteps = i.lot.productionOrder?.route?.steps ?? [];
      return <div className="card" key={i.id} style={{ marginBottom: 18 }}>
        ...
        {i.status === "DRAFT" && <>
          ...
          <div style={{ marginTop: 14 }}><strong>Nueva evidencia</strong><EntityDocumentUpload entityType="QUALITY_INSPECTION" entityId={i.id} defaultCategory="Evidencia de calidad" compact camera /></div>
          ...
          <form action={resolveInspection} className="form" style={{ marginTop: 14 }}>
            ...
            <div className="field full">
              <label>Reproceso: reiniciar desde</label>
              <select name="reworkRouteStepId" defaultValue="">
                <option value="">Selecciona la etapa sólo si vas a reprocesar</option>
                {routeSteps.map(step => <option key={step.id} value={step.id}>{step.sequence}. {step.process.name} ({step.process.code})</option>)}
              </select>
              <div className="muted">Al crear reproceso se generarán nuevamente esta etapa y todas las posteriores de la ruta.</div>
            </div>
            {routeSteps.length === 0 && <div className="field full"><div className="muted">Este lote no tiene una ruta disponible; no podrá generar una OP de reproceso trazable hasta asignarle una ruta.</div></div>}
            <div className="full" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button className="button" name="disposition" value="REWORK" disabled={routeSteps.length === 0}>Crear reproceso</button>
              <button className="button" name="disposition" value="REJECT">Rechazar</button>
            </div>
          </form>
        </>}

        <div style={{ marginTop: 16 }}><strong>Archivos protegidos</strong><div style={{ marginTop: 8 }}><DocumentList documents={docs} emptyText="Sin evidencia protegida." /></div></div>
